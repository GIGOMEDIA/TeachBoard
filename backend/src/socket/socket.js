...
export const initSocket = (io) => {
  io.on('connection', (socket) => {
    ...
    // Broadcast canvas background change
    socket.on('canvas-background-change', (backgroundType) => {
      if (socket.roomCode) {
        socket.to(socket.roomCode).emit('canvas-background-change-received', backgroundType);
      }
    });
