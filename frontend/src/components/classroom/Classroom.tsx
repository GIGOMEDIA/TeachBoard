...
export default function Classroom({ setView }: ClassroomProps) {
  ...
  // Toggle request to speak (Student)
  const handleToggleRequestSpeak = () => {
    if (isBroadcastingVoice) {
      stopLocalVoice();
      stopVoiceBroadcast();
      setIsBroadcastingVoice(false);
    } else if (isVoicePending) {
      cancelRequestSpeak();
  ...
  React.useEffect(() => {
    ...
    // Handle student speak approvals
    socket.on('voice-approved', async () => {
      console.log('Voice: Speak request approved!');
      setIsVoicePending(false);
      try {
        setVoiceError(null);
        const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
        ...
        console.error('Voice: Failed to capture approved speaker microphone', err);
    ...
    socket.on('voice-rejected', () => {
      console.log('Voice: Request rejected.');
      setIsVoicePending(false);
      alert('Your request to speak was declined by the instructor.');
    });

    socket.on('voice-revoked', () => {
      console.log('Voice: Speaking revoked.');
      stopLocalVoice();
      stopVoiceBroadcast();
      setIsBroadcastingVoice(false);
      setIsVoicePending(false);
      alert('Your speaking session has been ended by the instructor.');
    });

    return () => {
      socket.off('voice-signal-received');
      socket.off('voice-approved');
      socket.off('voice-rejected');
      socket.off('voice-revoked');
    };
  }, [socket, startVoiceBroadcast, stopVoiceBroadcast, cancelRequestSpeak, stopLocalVoice]);
