...
export const useStore = create<TeachBoardState>((set, get) => ({
  ...
  connectRoom: (roomCode) => {
    ...
    newSocket.on('canvas-background-change-received', (type: 'grid' | 'blank' | 'coordinate') => {
  ...
  setBackgroundType: (type) => {
    ...
    if (isConnected && socket && user?.role === 'teacher') {
      socket.emit('canvas-background-change', type);
    }
  },
